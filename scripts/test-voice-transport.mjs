...
voice.on('connection',ws=>{sidecar=ws;ws.send(JSON.stringify({type:'voice_ready'}));ws.on('message',raw=>{const event=JSON.parse(raw);voiceEvents.push(event);if(event.type==='speak')ws.send(JSON.stringify({type:'voice_audio',audio:'AAA=',sampleRate:48000,requestId:event.requestId}));});});
...
let logs='';
const child=spawn(process.execPath,['--import','tsx','server.ts'],{env:{...process.env,NODE_ENV:'production',PORT:String(port),GRADIUM_API_KEY:'local-test',GRADIUM_VOICE_ID:'local-test',GENERAL_COMPUTE_API_KEY:'',SAMBANOVA_API_KEY:'',MEMORIES_API_KEY:'',JEV_API_KEY:'',CORTEX_MODE:'DEMO',PIPECAT_API_URL:`http://127.0.0.1:${voice.address().port}`,MUJOCO_API_URL:`http://127.0.0.1:${robot.address().port}`},stdio:['ignore','pipe','pipe']});
...
try{
 ...
 sidecar.send(JSON.stringify({type:'voice_audio',audio:'AAA=',sampleRate:48000,requestId:voiceEvents.find(e=>e.type==='speak').requestId}));await wait(150);assert.ok(!events.some(e=>e.type==='voice_audio'),'Late audio must be blocked');
