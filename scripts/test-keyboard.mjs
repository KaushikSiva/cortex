...
try{
 await page.goto('http://localhost:3000',{waitUntil:'networkidle'});await wait(async()=>!!(await state()).robot);await reset();
 ...
 await page.waitForTimeout(900);await page.keyboard.up('a');await wait(async()=>(await state()).robot.status==='idle');expect((await state()).robot.pose.y).toBeGreaterThan(.2);
 ...
 await page.keyboard.press('Space');await reset();await page.locator('input').fill('move left 1 meter');await page.locator('input').press('Enter');await wait(async()=>(await state()).robot.currentWaypoint==='DIRECTIONAL'&&(await state()).robot.motion==='walk');expect((await state()).plan[0].name).toBe('walk');
