...
try{
 ...
 const state=await(await fetch('http://localhost:3000/api/status')).json();let socket,epoch=0;
 ...
 const publish=e=>socket.send(JSON.stringify({type:'voice_audio',audio:pcm.toString('base64'),sampleRate:48000,voiceEpoch:e,requestId:'test'}));
