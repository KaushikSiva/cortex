...
try{
 ...
 send({type:'fixture',name:'stop'});await until(()=>state.robot.estop);await until(()=>state.metrics.some(m=>m.name==='Stop → settled'&&m.ms!==null));const stopMetrics=state.metrics;
 ...
 send({type:'stop'});await until(()=>state.robot.estop);send({type:'text',text:'Continue, but slowly.'});await until(()=>state.robot.status==='moving');assert.equal(state.policy.maxSpeed,.3);
 send({type:'stop'});await until(()=>state.robot.estop);const result={passed:true,calmXAfter4s:calm,urgentXAfter4s:urgent,stopMetrics,tests:['same-word physical difference','fast stop','fearful confirmation','resume slowly'],timestamp:new Date().toISOString()};await writeFile('docs/integration-results.json',JSON.stringify(result,null,2));console.log(result);
