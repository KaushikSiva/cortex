...
wss.on('connection',ws=>{
 ...
 let speechGeneration=0,audioAllowed=false;
 function interruptSpeech(){
  speechGeneration++;audioAllowed=false;runtime.interruptConversation();
 ...
 function speakReply(generation:number){
  ...
  audioAllowed=true;
  ws.send(JSON.stringify({type:'voice_reply',text:runtime.state.response}));
  gradium.send({type:'speak',text:runtime.state.response});
 }
 const gradium=new GradiumBridge(raw=>{
  const event=raw as {type?:string;message?:string;interim?:boolean;transcript?:string};
  if(event.type==='voice_audio'){if(audioAllowed&&ws.readyState===WebSocket.OPEN)ws.send(JSON.stringify(raw));return;}
