...
test('stop cancels movement but accepts the next navigation without resume',async()=>{const f=backend(),g=new SafetyGovernor(f.b);await g.stop();await g.navigate('PERSON',defaultPolicy);assert.deepEqual(f.calls,['stop','navigate']);});
test('stale telemetry stops, stale motion rejected, obstacles stop',async()=>{const f=backend(),g=new SafetyGovernor(f.b);await assert.rejects(g.navigate('PERSON',defaultPolicy,Date.now()-5000));f.snapshot={...state(),timestamp:Date.now()-3000};await assert.rejects(g.navigate('PERSON',defaultPolicy));assert.ok(f.calls.includes('stop'));await g.reset();f.snapshot={...state(),nearestObstacle:.1};await assert.rejects(g.navigate('PERSON',defaultPolicy));assert.ok(f.calls.includes('stop'));});
...
test('a stop racing reset cannot be cleared by reset completion',async()=>{const f=backend();let release!:()=>void;f.b.reset=()=>new Promise(r=>{release=r});const g=new SafetyGovernor(f.b);const reset=g.reset();await g.stop();release();await reset;assert.ok(f.calls.includes('stop'));assert.deepEqual(f.calls,['stop','stop']);});
...
test('stop in a spoken confirmation cancels the held mission without resuming',async()=>{
 ...
 assert.equal(rt.state.pendingConfirmation,false);assert.equal(rt.state.phase,'READY');
