...
export class VisualMemory{
 ...
 async remember(image:string,live:boolean){
  ...
  if(live){
   ...
   const form=new FormData();form.append('files',new Blob([buffer],{type:'image/png'}),filename);form.append('unique_id','cortex-yerba-buena');
   ...
   if(!response.ok)throw new Error(`Memories.ai upload HTTP ${response.status}`);const raw=await response.json();event.raw=raw;
 ...
 async searchVisualMemory(query:string,live:boolean):Promise<MemoryEvent[]>{
  ...
  if(!process.env.MEMORIES_API_KEY)throw new Error('Memories.ai API key missing');
  const response=await fetch('https://api.memories.ai/serve/api/v1/search',{method:'POST',headers:{Authorization:process.env.MEMORIES_API_KEY,'Content-Type':'application/json'},body:JSON.stringify({search_param:query,unique_id:'cortex-yerba-buena',top_k:5}),signal:AbortSignal.timeout(12000)});
  if(!response.ok)throw new Error(`Memories.ai search HTTP ${response.status}`);const raw=await response.json();
