...
export async function plan(transcript:string,context:unknown,live:boolean):Promise<ToolCall[]>{
 if(!live){
  ...
  if(/\bdance\b/i.test(transcript))return [{name:'dance',arguments:{beats:4}}];
 ...
 const provider=inferenceConfig();if(!provider)throw new Error('Configure General Compute or SambaNova for inference');
 ...
 const r=await fetch(provider.endpoint,{method:'POST',headers:{Authorization:`Bearer ${provider.apiKey}`,'Content-Type':'application/json'},body:JSON.stringify({model:provider.model,messages:[{role:'system',content:'You are CORTEX mission planner. Use tools only. turn(angleDegrees) is relative: left positive, right negative, back 180. walk(direction, meters) internally reuses turn; do not add a turn before a directional walk. Use walk_to(target) for explicit named destinations. Keyboard and voice share these exact tools. Search memory before navigating for objects. Never invent sightings. Come here means PERSON. Continue resumes robot.currentWaypoint. Do not navigate for a question asking only where. Requests to take/bring the user there, help find the object, or saying they really need it request navigation after retrieval. If context.memoryResolved is true, search already completed: use only that evidence and its waypoint; do not search again. Otherwise return search_memory alone for an object mission; its result will be provided in the next planning step. Context: '+JSON.stringify(context)},{role:'user',content:transcript}],tools,tool_choice:'required',temperature:.1,max_tokens:provider.maxTokens}),signal:AbortSignal.timeout(12000)});
 if(!r.ok)throw new Error(`${provider.name} HTTP ${r.status}`);const data=await r.json();const calls=data.choices?.[0]?.message?.tool_calls;
