...
export function validateMemoryFollowup(calls:ToolCall[],waypoint:string):ToolCall[]{
 ...
 if(resolved.filter(c=>c.name==='navigate_to').length!==1||resolved.some(c=>!['navigate_to','speak'].includes(c.name)||(c.name==='navigate_to'&&c.arguments.location!==waypoint)))throw new Error('Memory plan must navigate only to the retrieved evidence waypoint');
 return resolved;
}
