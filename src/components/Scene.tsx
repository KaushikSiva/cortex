...
export default function Scene({robot,onCanvas}:{robot:RobotState|null;onCanvas:(canvas:HTMLCanvasElement)=>void}){
 ...
 useEffect(()=>{
  ...
  const camera=new THREE.PerspectiveCamera(60,1,.05,150);camera.up.set(0,0,1);camera.position.set(1,-5.8,1.65);camera.lookAt(0,18,1.8);
  ...
  let inspection=false;const inspect=(event:Event)=>{const detail=(event as CustomEvent).detail;inspection=typeof detail==='boolean'?detail:!inspection;controls.enabled=inspection;setInspecting(inspection);if(!inspection){camera.position.set(1,-5.8,1.65);controls.target.set(0,18,1.8);camera.lookAt(controls.target);controls.update();}};window.addEventListener('cortex-inspect',inspect);
