...
export function useMicrophone(send:(message:unknown)=>void){
 ...
 const media=useRef<MediaStream|null>(null),context=useRef<AudioContext|null>(null),node=useRef<AudioWorkletNode|null>(null),pending=useRef(false),playAt=useRef(0),sources=useRef<AudioBufferSourceNode[]>([]);
 function silence(){for(const source of sources.current){try{source.stop();}catch{}}sources.current=[];playAt.current=0;}
 ...
 function play(audio:string,sampleRate:number){
  const ctx=context.current;if(!ctx)return;
 ...
 return {prepare,begin,stop,play,silence,listening,level};
}
