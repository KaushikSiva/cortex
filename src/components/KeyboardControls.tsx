...
type Props={send:(message:unknown)=>void;connected:boolean};
...
export function KeyboardControls({send,connected}:Props){
 ...
 const press=useCallback((direction:Direction,key:string)=>{if(!connected)return;if(held.current?.key===key)return;release();const session=crypto.randomUUID();held.current={session,key};setActive(direction);send({type:'manual_start',direction,session});},[send,connected,release]);
 ...
 const button=(direction:Direction,label:string,Icon:typeof ArrowUp)=><button key={direction} aria-label={`Walk ${direction}`} className={active===direction?'held':''} disabled={!connected} onPointerDown={e=>{e.preventDefault();e.currentTarget.setPointerCapture(e.pointerId);press(direction,'pointer-'+direction);}} onPointerUp={release} onPointerCancel={release} onLostPointerCapture={release}><Icon size={15}/><kbd>{label}</kbd></button>;
 return <section className="motion-console" aria-label="Robot movement controls">
  ...
  <div className="turn-controls"><span className="eyebrow">TURN IN PLACE</span><div><button onClick={()=>turn(90)} disabled={!connected} aria-label="Turn left 90 degrees"><RotateCcw size={14}/>Left <kbd>Q</kbd></button><button onClick={()=>turn(-90)} disabled={!connected} aria-label="Turn right 90 degrees"><RotateCw size={14}/>Right <kbd>E</kbd></button></div><small>Robot-relative directions · Space stops.</small></div>
  <div className="target-controls"><span className="eyebrow">WALK TO TARGET</span><div><select aria-label="Navigation target" value={target} onChange={e=>setTarget(e.target.value)}>{['HOME','PERSON','PLANTER','BENCH','DOOR'].map(p=><option key={p}>{p}</option>)}</select><button disabled={!connected} onClick={()=>{release();send({type:'tool',name:'walk_to',arguments:{target}});}}>Go <ArrowRight size={14}/></button></div><small>Shared motion tools</small></div>
 </section>;
}
