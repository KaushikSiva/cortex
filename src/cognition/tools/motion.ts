...
type Context={active:()=>boolean;confirmed?:boolean;session?:string;trace:(message:string)=>void};
...
export class MotionTools{
 ...
 async turn(angleDegrees:number,policy:BehaviorPolicy,context:Context){
  ...
  await this.safety.turn(yaw,policy,!!context.confirmed,context.session);
 ...
 async walk(direction:Direction,meters:number,policy:BehaviorPolicy,context:Context){
  ...
  if(context.session){await this.safety.drive(yaw,policy,context.session);context.trace(`walk(${direction}) · held key`);}
  else {await this.safety.walk(yaw,meters,policy,!!context.confirmed);context.trace(`walk(${direction}, ${meters.toFixed(1)} m)`);}
 }
 ...
 async walkTo(target:string,policy:BehaviorPolicy,context:Context){
  ...
  await this.safety.navigate(waypoint,policy,Date.now(),!!context.confirmed);context.trace(`walk_to(${waypoint})`);
 }
}
