...
export class SafetyGovernor{
 ...
 async stop(){this.generation++;await this.backend.stop();}
 async reset(){const token=++this.generation;await this.backend.reset();if(this.generation!==token){await this.backend.stop();return;}}
 async resume(){const token=++this.generation;await this.backend.resume();if(this.generation!==token){await this.backend.stop();return;}}
 ...
 private async motionGate(policy:BehaviorPolicy,issuedAt:number,confirmed:boolean){
  ...
  if(token!==this.generation)throw new Error('Motion superseded by stop');
  if(Math.abs(Date.now()-s.timestamp)>750){await this.stop();throw new Error('Stale telemetry');}
  if(s.status==='error')throw new Error('Robot is not ready');
 ...
 private async afterMotion(token:number){if(token!==this.generation){await this.backend.hold();}}
 ...
 async setSpeed(speed:number){z.number().finite().nonnegative().parse(speed);await this.backend.setSpeed(Math.min(speed,.9));}
}
