// Adapted from the user's chennai-gta solar-separated daylight renderer.
 ...
 */
export function prepareDaylight(source:THREE.DataTexture,renderer:THREE.WebGLRenderer) {
  ...
  return {background:source,environment:target.texture,target,direction,rotation,intensity,color,
    evidence:{source:'https://polyhaven.com/a/kloofendal_48d_partly_cloudy',license:'CC0',authors:['Greg Zaal'],size:[width,height],peakPixel:[peak%width,Math.floor(peak/width)],rawDirection:rawDirection.toArray(),direction:direction.toArray(),rotationY:rotation,solarRadiusDegrees:3,backgroundAnnulusDegrees:[3,5],solarPixels:solar.length,annulusRGB:fillRGB,removedIntegratedRGB:removedRGB,lightIntensity:intensity,lightColor:color.toArray(),limits:'Illustrative sky, not captured at Alamo Square. Azimuth inferred from facade shadows; solar elevation comes from this HDR, not a calibrated date or time. A 3-degree solar/halo core is approximated by a directional light; indirect occlusion and bounce remain incomplete.'}};
}
